...
async fn permission_check_empty_denies() {
    ...
    assert!(!dingtalk.check_permission("user1", "张三"));
...
/// 测试昵称匹配
#[tokio::test]
async fn permission_check_nickname_match() {
    let config = DingTalkConfig {
        enabled: false,
        client_id: "test_client_id".to_string(),
        client_secret: "test_client_secret".to_string(),
        allow_from: vec!["张三".to_string(), "user2".to_string()],
    };
    ...
    // 昵称匹配
    assert!(dingtalk.check_permission("staff_123", "张三"));
    // ID 匹配
    assert!(dingtalk.check_permission("user2", "李四"));
    // 都不匹配
    assert!(!dingtalk.check_permission("staff_456", "王五"));
}
