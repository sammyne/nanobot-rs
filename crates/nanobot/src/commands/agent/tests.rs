...
fn agent_cmd_default() {
    let cmd = AgentCmd { message: None, image: None, session: "cli:direct".to_string() };
...
fn agent_cmd_with_message() {
    let cmd = AgentCmd { message: Some("Hello".to_string()), image: None, session: "telegram:12345".to_string() };
